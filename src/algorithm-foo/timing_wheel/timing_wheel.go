...
func (mtw *ModuleTimingWheel) Mounted() {
	mtw.Logger().Debug("OBSERVE: Mounted, mount series timing wheel rounds", zap.Any("roundsCount", len(mtw.rounds)))
...
func (mtw *ModuleTimingWheel) mountSeriesTimingWheelRound(rounds ...*timingWheelRound) error {
	...
	// 加上无限时间轮
	_rounds = append(_rounds, roundInfinite)
	// 设置
	mtw.pointer = newTimingWheelPointer(newSeriesTimingWheelDisc(_rounds...))
	return nil
}
...
func (mtw *ModuleTimingWheel) HandleEvent(event *sgs.ModuleEvent) {
	...
	default:
		// 处理 原生 ModuleEvent
		mtw.Logger().Error(sgs.ErrorMsgHandleEventNonImplement)
	}
}
...
func (mtw *ModuleTimingWheel) Run() {
	...
	go mtw.pointer.start(mtw)
}

func (mtw *ModuleTimingWheel) Unmounted() {
	mtw.Logger().Debug("OBSERVE: Unmounted")
}
...
func (mtw *ModuleTimingWheel) handleTimingBehavior(timingBehavior ITickHandler) {
	...
	if err != nil {
		mtw.Logger().Error("handleTimingBehavior, addTickerHandler occurs error", zap.Error(err), zap.Any("timingBehavior", timingBehavior))
		return
	}
}
