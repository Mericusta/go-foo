...
import (
	builtinSync "go-foo/src/builtin-foo/src/sync"
	"runtime"
	"sync"
	"sync/atomic"
	"testing"
...
func TestPoolDequeue(t *testing.T) {
	testPoolDequeue(t, builtinSync.NewPoolDequeue(16))
}

func TestPoolChain(t *testing.T) {
	testPoolDequeue(t, builtinSync.NewPoolChain())
}

func testPoolDequeue(t *testing.T, d builtinSync.PoolDequeue) {
	const P = 10
	var N int = 2e6
	if testing.Short() {
		N = 1e3
