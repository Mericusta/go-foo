...
type timingWheelDisc struct {
	...
	prev  *timingWheelDisc  // 上一级轮，必须必当前轮的刻度小
}
...
func newTimingWheelDisc(round *timingWheelRound, next *timingWheelDisc) *timingWheelDisc {
	var slots [][]ITickHandler
	if round.duration == 0 {
		slots = make([][]ITickHandler, 0, round.slot)
	} else {
	...
	// TODO: 设置初始 roundPosition 为 round.slot，在首次运行时直接触发进位
	twd := &timingWheelDisc{round: round, slots: slots, next: next, roundPosition: round.slot}
...
func (twd *timingWheelDisc) tick(pointer *timingWheelPointer) []ITickHandler {
	...
	if twd.roundPosition == twd.round.slot {
		...
		if twd.next != nil {
			...
			// 从无限时间轮中获取若干可以放置到前一级时间轮下的行为
			handlers := make([]ITickHandler, 0, 64)
			clearIndex := -1
			for index, slot := range twd.slots {
				if len(slot) > 0 {
					if slot[0].Delay() < twd.prev.round.duration {
						handlers = append(handlers, slot...)
					}
				}
				clearIndex = index
			}
			if clearIndex != -1 {
				twd.slots = slices.Delete(twd.slots, 0, clearIndex+1)
			}
			return handlers
		}
	}
...
func (twd *timingWheelDisc) place(handler ITickHandler) {
	...
	if twd.round.duration != 0 {
		// 超过当前周期的总时长
		if relativeDuration >= twd.round.duration {
			// 进位时加上当前的偏移量
			handler.Update(relativeDuration - twd.round.duration)
		...
		// 放入到指定位置时需要更新 handler 的延迟，以应对进位的情况
		handler.Update(relativeDuration - time.Duration(placePosition)*twd.round.tick)
		twd.slots[placePosition] = append(twd.slots[placePosition], handler)
	} else {
		// TODO: 需要测试无限时间轮
		// 插入排序法由小到大放入到无限时间轮中
		insertIndex, insertSameDuration := -1, false
		for index, slot := range twd.slots {
			if len(slot) > 0 && slot[0] != nil {
				if handler.Delay() < slot[0].Delay() {
					insertIndex = index
				} else if handler.Delay() == slot[0].Delay() {
					insertSameDuration = true
					twd.slots[index] = append(twd.slots[index], handler)
				}
			}
		}
		if !insertSameDuration {
			if insertIndex == -1 {
				twd.slots = append(twd.slots, []ITickHandler{handler})
			} else {
				twd.slots = slices.Insert(twd.slots, insertIndex, []ITickHandler{handler})
			}
		}
	}
}
...
func newTimingWheelPointer(disc *timingWheelDisc) *timingWheelPointer {
	return &timingWheelPointer{
		...
		disc:     disc,
		receiver: make(chan ITickHandler, 512), // 每个 tick 超过 256 个行为可能会导致精度下降（TODO: 这里应该是所有 disc 的可接收数量加起来，暂时使用最低精度的）
		// TODO: 暂时修改成 512 以处理机器人 handle event 阻塞转嫁而来的问题
	}
}
...
func (tmp *timingWheelPointer) start(ctx sgs.IModuleEventContext) {
	...
	for {
		...
		case <-ctx.Module().Base().Ctx().Done():
			// fmt.Printf("trigger tick stop at counter %v\n", tmp.counter.Load())
			tmp.ticker.Stop()
			return
		}
	}
...
func (tmp *timingWheelPointer) addTickerHandler(ths ...ITickHandler) error {
	...
	for _, handler := range ths {
		...
		default:
			fmt.Printf("OBSERVE: timingWheelPointer.AddTickerHandler blocked\n")
		}
	}
	return nil
}
